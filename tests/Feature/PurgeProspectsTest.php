...
it('purge supprime les non contactés + ignorés et préserve le reste', function () {
    p(['statut' => 'a_contacter']);                 // supprimé
    p(['statut' => 'ignore']);                      // supprimé (ignoré)
    ...
    $deleted = Prospect::whereIn('statut', ['a_contacter', 'ignore'])
        ->where('est_client', false)->delete();

    expect($deleted)->toBe(2)
        ->and(Prospect::count())->toBe(3)
        ->and(Prospect::whereIn('statut', ['a_contacter', 'ignore'])->where('est_client', false)->count())->toBe(0)
